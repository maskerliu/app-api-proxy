...
export class BaseRouter {
  ...
  protected async route(req: Request, resp: Response, func: Function, target: any,
    paramInfos: Array<{ key: string, val: ParamType }>) {

    let params = paramInfos.map(item => this.parseParam(req, item.key, item.val))

    let bizResp: BizResponse<any>
    try {
      let result = await Reflect.apply(func, target, [...params, this.parseContext(req)])
      if (result == null) {
  ...
  private parseContext(req: Request): BizContext {
    ...
    if (ua == null) {
      ...
      console.log(regArr)
      const scheme = regArr[0].trim() // Mozilla/5.0 
      const os = regArr[1].trim().substring(1, regArr[1].length - 1).split(';')[0] // (Windows NT 10.0; Win64; x64)
      const [platform, _] = regArr[4].trim().split(' ') // Chrome/131.0.0.0 Safari/537.36
