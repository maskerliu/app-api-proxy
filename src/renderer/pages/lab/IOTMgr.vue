  ...
  <van-row class="full-row" justify="center" align="center">
    <van-col class="full-row bg-border" style="max-height: 174px;">
      ...
      <van-grid clickable style="overflow-y: auto;  overflow-x: hidden;">
        <van-grid-item v-for="item in devices" @click="onDeviceSelected(item)" badge="">
          <template #text>
            <div class="single-line">{{ item.deviceId }}</div>
            <div class="single-line">地址: {{ item.address }}</div>
            <div>
              <van-button type="warning" plain size="mini"
    ...
    <van-row class="full-row bg-border" style="height: 50px;" align="center">
      <van-col span="20">
        {{ curDevice?.deviceId }}
      </van-col>
      <van-col>
    ...
    <van-row class="full-row bg-border" justify="center"
    ...
    <van-popup v-model:show="showInfoDialog" :close-on-click-overlay="false" style="padding: 5px;">
      <van-form style="margin-top: 15px; width: 100%" label-align="right" label-width="5em" colon>
