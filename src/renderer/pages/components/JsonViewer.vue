...
const prefix = computed((): string => { return isArray.value ? '[' : '{' })

const items = computed(() => {
  const data = props.data
  if (isArray.value) {
    return data.map((item: any) => {
      return { value: item, isJSON: isObjectOrArray(item), key: '' }
    })
  }
...
const iconColors = computed(() => { return ['#747983', '#747983'] })

onMounted(() => {
  innerclosed.value = props.closed
  templateDeep.value = props.currentDeep
  nextTick(() => {
    visible.value = true
  })
})
...
function isClose(curDeep: number) {
  return curDeep > props.deep
}

function isEmptyArrayOrObject(data: any) {
  // 空数组或者空对象
  return [{}, []]
